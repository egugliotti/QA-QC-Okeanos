...
### The loop to test if observations fall within 20 m buffer ###

buf.wgs$Dive<-str_replace_all(buf.wgs$dive,"DIVE", "Dive") # Need to replace DIVE with Dive so that buf.wgs$Dive matches indata$EventID
indata_sf<-st_as_sf(x=indata, coords=c("Longitude","Latitude"), crs=st_crs(buf.wgs)) # Make indata an sf class with crs same as buf.wgs
indata_coords <- do.call(rbind, st_geometry(indata_sf)) %>% 
     as_tibble() %>% setNames(c("lon","lat")) # Separate sf geometry to get lat and lon for map
indata_sf$Lon<-indata_coords$lon # Put lat and long in indata_sf
...
# Filter for only observations that don't fall within the 20 m buffer. These should be the same as in the process below
buffer.df<- buffer.df %>% 
  dplyr::filter(in.buffer==FALSE)



### Just in case the loop doesn't work ###
# by dive
# Dive 1
